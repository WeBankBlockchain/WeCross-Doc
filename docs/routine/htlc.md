...
[WeCross.org1-admin]> bcosDeploy payment.bcos.ledger contracts/solidity/LedgerSample.sol LedgerSample 1.0 token htlc 1 100000000

# 资产合约地址需要记录下来
Result: 0xf4fdcdfe0184644f09a1cfa16a945cc71a5d44ff

# 部署htlc合约
[WeCross.org1-admin]> bcosDeploy payment.bcos.htlc contracts/solidity/LedgerSampleHTLC.sol LedgerSampleHTLC 1.0
...
[WeCross.org1-admin]> sendTransaction payment.bcos.ledger approve 0x22a83719f748da09845d91fe1a2f44437f0ad13b 1000000
...
# init [己方资产合约地址] 
[WeCross.org1-admin]> sendTransaction payment.bcos.htlc init 0xf4fdcdfe0184644f09a1cfa16a945cc71a5d44ff
...
# 查看资产发行者owner的地址
[WeCross.org1-admin]> call payment.bcos.htlc queryAddress

# 该地址需要记录下来，发起转账提案时需要
Result: [0x55f934bcbe1e9aef8337f5551142a442fdde781c]

# 查看owner余额，检查是否初始化成功
[WeCross.org1-admin]> call payment.bcos.htlc balanceOf 0x55f934bcbe1e9aef8337f5551142a442fdde781c
...
# 切换默认账户
[WeCross.org2-admin]>setDefaultAccount Fabric1.4 2

# 在机构2安装资产合约链码
[WeCross.org2-admin]> fabricInstall payment.fabric.ledger Org2 contracts/chaincode/ledger 1.0 GO_LANG

path: classpath:contracts/chaincode/ledger
Result: Success

# 在机构2安装哈希时间锁合约链码
[WeCross.org2-admin]> fabricInstall payment.fabric.htlc Org2 contracts/chaincode/htlc 1.0 GO_LANG

path: classpath:contracts/chaincode/htlc
Result: Success

# 切换默认账户
[WeCross.org2-admin]>setDefaultAccount Fabric1.4 1

# 在机构1安装资产合约链码
[WeCross.org2-admin]> fabricInstall payment.fabric.ledger Org1 contracts/chaincode/ledger 1.0 GO_LANG

path: classpath:contracts/chaincode/ledger
Result: Success

# 在机构1安装哈希时间锁合约链码
[WeCross.org2-admin]> fabricInstall payment.fabric.htlc Org1 contracts/chaincode/htlc 1.0 GO_LANG

path: classpath:contracts/chaincode/htlc
Result: Success

# 实例化链码，发行资产100000000
[WeCross.org2-admin]> fabricInstantiate payment.fabric.ledger ["Org1","Org2"] contracts/chaincode/ledger 1.0 GO_LANG default ["token","htlc","100000000"]

Result: Query success. Please wait and use 'listResources' to check.

# 用listResources确认payment.fabric.ledger已实例化完成（约1分钟）
[WeCross.org2-admin]> listResources

path: payment.fabric.ledger, type: Fabric1.4, distance: 0

# 实例化哈希时间锁合约，需要写入[己方资产合约名，channel]
[WeCross.org2-admin]> fabricInstantiate payment.fabric.htlc ["Org1","Org2"] contracts/chaincode/htlc 1.0 GO_LANG default ["ledger","mychannel",]
...
# 创建一个托管账户完成授权
[WeCross.org2-admin]> sendTransaction payment.fabric.ledger createEscrowAccount 1000000

Txhash  : d6a6241cbaac9cad768465213f3462f54c62e32f168c26bcce23d060315f0751
BlockNum: 1097
Result  : [HTLCoin-Admin@org2.example.com-EscrowAccount]

# 查看资产发行者owner的地址
[WeCross.org2-admin]> call payment.fabric.htlc queryAddress

Result: [Admin@org2.example.com]

# 查看owner授权后的余额
[WeCross.org2-admin]> call payment.fabric.htlc balanceOf Admin@org2.example.com
...
- FISCO BCOS转账金额：700
- Fabric的两个账户：
    - 资产转出者：Admin@org2.example.com （Fabric链发行资产的地址）
...
[WeCross.org1-admin]> newHTLCProposal payment.bcos.htlc edafd70a27887b361174ba5b831777c761eb34ef23ee7343106c0b545ec1052f 049db09dd9cf6fcf69486512c1498a1f6ea11d33b271aaad1893cd590c16542a true 0x55f934bcbe1e9aef8337f5551142a442fdde781c 0x2b5ad5c4795c026514f8317c7a215e218dccd6cf 700 2000010000 Admin@org2.example.com User1@org1.example.com 500 2000000000
```
...
[WeCross.org2-admin]> newHTLCProposal payment.fabric.htlc edafd70a27887b361174ba5b831777c761eb34ef23ee7343106c0b545ec1052f null false 0x55f934bcbe1e9aef8337f5551142a442fdde781c 0x2b5ad5c4795c026514f8317c7a215e218dccd6cf 700 2000010000 Admin@org2.example.com User1@org1.example.com 500 2000000000
```
...
Result: [500]
```
**注**：跨链转账存在交易时延，取决于两条链以及机器的性能，一般需要5~25s完成转账。Fabric链资产转出者Admin@org2.example.com在授权的时候已经把钱转移到了托管账户，因此htlc扣的是托管账户，他的余额不会发生变化。
