...
本文分为[部署手册](#部署手册)和[使用手册](#使用手册)两部分，部署手册介绍WeCross网页管理平台的部署启动方式，使用手册介绍WeCross网页管理平台的实际操作和使用方式。
...
- 为了您有最好的操作体验，请尽量使用[Chrome浏览器](https://www.google.cn/chrome/)，或使用支持ES2015语法的浏览器，可参考链接：[caniuse](https://caniuse.com/es6)。*若使用过时的浏览器，有可能会出现部分功能不可使用的问题 :(*
- 若您需要进行手动源码编译网页管理平台，请确保在您的编译环境中`node.js`和`npm`版本支持ES2015，可参考链接：[node.green](https://node.green/)。推荐版本为`node.js >= 8.10, npm >= 6.4.1`
...
网页管理平台的启动方式分为两种方式：[快速体验](#快速体验)、[手动编译](#手动编译)。
...
![firstMet](../images/webApp/firstMet.png)
...
6. 在浏览器中输入`[跨链路由IP]:[跨链路由端口]/s/index.html`访问WeCross网页管理平台，请注意[浏览器版本要求](#环境要求)。
...
- 事务管理模块：查看所有跨链事务详情，可在页面开启/执行/结束新的事务；
...
![register](../images/webApp/register.png)
...
![homepage](../images/webApp/homepage.png)
...
![resourceManager](../images/webApp/resourceManager.png)

`资源列表视图`展示出某条脸上的所有资源，包括路径、类型、具体属性的字段，可对某个资源进行操作，包括`发交易`和`查状态`两种操作。
...
![callResource](../images/webApp/callResource.png)

在`资源管理`界面点击`部署合约`按钮，可进入`资源部署页面`。`资源部署页面`会以点击`部署合约`按钮时选择的链类型改变而改变。目前总共分为两大类链类型：Hyperledger Fabric和FISCO BCOS。

FISCO BCOS 资源部署页面如下图所示，操作可分为`部署合约`和`注册合约`。在红框1内填入部署的资源命名。**注意：** 在上传合约文件是，必须上传所有合约文件打包的`ZIP`压缩包，并且必须在文件夹最外层有`.sol`或`.abi`的合约文件。上传之后，红框2的`合约入口文件`下拉框会列出压缩包内最外层的所有合约文件，在确认执行部署之后，网页会自动读取`合约入口文件`，并解析合约之间的依赖，所以必须在压缩包内包含传入合约的依赖合约。在合约类名中填入`合约入口文件`的类名。

![BCOSDeploy](../images/webApp/BCOSDeploy.png)
...
![FabricDeploy](../images/webApp/FabricDeploy.png)
...
![FabricInstan](../images/webApp/FabricInstan.png)

##### 交易管理模块

用户可通过交易管理模块查看所有跨链资源的每一笔交易详情，进行跨链资源调用。界面如下图所示，与[资源管理模块](#资源管理模块)相似，分为两大块，右边蓝框1内为`交易列表视图`，展示所选定的链所有交易信息，包含交易哈希、账户、区块高度、资源、调用方法和回执等信息，点击蓝框2内可查看这一行交易的回执详细信息。

![transactionManager](../images/webApp/transactionManager.png)
...
![xaManager](../images/webApp/xaManager.png)
